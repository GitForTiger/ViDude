...
# IMPORT YOUR NEW CONTROLLER HERE
from core.transcriberController import process_transcription
...
with st.sidebar:
    ...
    # Removed the language selectbox here!
    ...
    if st.session_state.pipeline_done:
        ...
        for step, icon, label in [
            ...
            ("transcript", "📝", "Transcription (Auto)"), # Added a nice visual cue that it's auto-detecting
...
# ─── Main Area ──────────────────────────────────────────────────────────────────
st.markdown('<div class="hero-title">ViDude : AI Video Assistant</div>', unsafe_allow_html=True)
...
if run_btn:
    if not source.strip():
        ...
        try:
            ...
            # Called the new auto-detecting controller here!
            transcript = process_transcription(chunks)
...
if st.session_state.result:
    ...
    # Empty state (Removed the manual language prompt here!)
    st.markdown("""
